...
kernel_do_install() {
	...
	install -d ${D}/boot
	install -m 0644 ${KERNEL_OUTPUT} ${D}/${KERNEL_IMAGEDEST}/vmlinuz-${KERNEL_VERSION}
	...
	install -m 0644 ${KERNEL_OUTPUT} $kerneldir/vmlinuz
...
pkg_postinst_kernel-base () {
	cd /${KERNEL_IMAGEDEST}; update-alternatives --install /${KERNEL_IMAGEDEST}/vmlinuz vmlinuz vmlinuz-${KERNEL_VERSION} ${KERNEL_PRIORITY} || true
}

pkg_postrm_kernel-base () {
	cd /${KERNEL_IMAGEDEST}; update-alternatives --remove vmlinuz vmlinuz-${KERNEL_VERSION} || true
}
...
KERNEL_IMAGE_BASE_NAME ?= "vmlinuz-${PV}-${PR}-${MACHINE}-${DATETIME}"
# Don't include the DATETIME variable in the sstate package signatures
KERNEL_IMAGE_BASE_NAME[vardepsexclude] = "DATETIME"
KERNEL_IMAGE_SYMLINK_NAME ?= "vmlinuz-${MACHINE}"
