...
def rewrite_uri(uri, d):
    import re
    xc_repo_prefix = d.getVar("OPENXT_GIT_MIRROR", d, True)
...
def rewrite_uris(d):
    repo_action_delay = d.getVar("REPO_ACTION_DELAY", d, True)
    rewriten_uris = []
    ...
    freezer_file = d.getVar("FREEZER_URI_FILE", d, True)
    rewriten_uris_str = "\n".join(rewriten_uris)
    if changed:
...
def hash_locals(d):
    freezer_locals_file = d.getVar("FREEZER_LOCALS_FILE", d, True)
    src_uri = (d.getVar("SRC_URI", d, True) or "").split()
    hashes = []
    for url in src_uri:
